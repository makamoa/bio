...
if __name__ == '__main__':
    ...
    # datadir='data/ocean_ccd2/'
    # sample_type = 'B'
    # samplefile = sample_type + '2'
    # image_type = 'su' # ld lu sd su
    #     # mirrorfile = datadir + 'Mirror.csv'
    #     # sample = Spectra(samplefile, mirrorfile,plot_color=True)
    # im = Image(datadir + samplefile + image_type + '.tiff', regime='selective', fast=True)
    # print(im.RGB)
    # im.show()
    # exit(0)
    # sp = Spectra(datadir + samplefile + '.csv', datadir + 'mirr'+sample_type+'.csv')
    # sp.show()
    # sample=Sample(samplefile, 'mirr'+sample_type, folder=datadir, spectra_kargs={'normalized' : True},
    #               only_spectra=False, image_ext=image_type+'.tiff',
    #               thick_file='thicknesses.pkl')
    # sample.show()
    # SpectraTheory(sample.thickness, pmma_file='PMMA_nam_dispersion.txt').show()
    # plt.figure(figsize=[10,5])
    # SpectraTheory(sample.thickness, pmma_file='PMMA_nam_dispersion.txt').show_spectra()
    # sample.spectra.show_spectra()
    # plt.show()
    #####################################
    #sample.image.show()
    # refractive_indices = [1.4,2.5]
    # samples = SampleArray.generate_random_samples(refractive_indices=refractive_indices,mus=[250,400], sigmas=20, n_samples=200)
    # #refractive_indices=[1.3,1.5],mus=[350,400]
    # samples.show()
    # samples.show_color_space()
    # core_samples, labels = dbscan(samples.colors,eps=0.03,min_samples=2)
    # samples.show_color_space(labels)
    # clustering = OPTICS(min_samples=6).fit(samples.colors)
    # print(clustering.labels_)
    # samples.show_color_space(clustering.labels_)
    # # connectivity matrix for structured Ward
    # connectivity = kneighbors_graph(
    #     samples.colors, n_neighbors=1, include_self=False)
    # # make connectivity symmetric
    # connectivity = 0.5 * (connectivity + connectivity.T)
    # clustering = AgglomerativeClustering(
    #     linkage="single", affinity="l1",
    #     n_clusters=3, connectivity=connectivity).fit(samples.colors)
    # samples.show_color_space(clustering.labels_)
    # tsne = manifold.TSNE(n_components=2, init='random',
    #                      random_state=0, perplexity=20)
    # Y = tsne.fit_transform(samples.colors)
    # for i in range(len(refractive_indices)):
    #     idx = samples.Y == i
    #     plt.scatter(Y[idx, 0], Y[idx, 1], c=mpl_colors[i])
    # plt.show()
    #
    # core_samples, labels = dbscan(Y,eps=0.8,min_samples=3)
    # plt.scatter(Y[:, 0], Y[:, 1], c=labels)
    # plt.show()
    #
    # labels = mixture.GaussianMixture(n_components=3).fit_predict(samples.colors)
    # samples.show_color_space(labels)
    #
    # spectral = cluster.SpectralClustering(
    #     n_clusters=2, eigen_solver='arpack',
    #     affinity="nearest_neighbors")
    # labels = spectral.fit_predict(samples.colors)
    # samples.show_color_space(labels)
    #
    # spectral = cluster.KMeans(
    #     n_clusters=2)
    # labels = spectral.fit_predict(Y)
    # samples.show_color_space(labels)
    #
    # Y = manifold.Isomap(n_components=2).fit_transform(samples.colors)
    # for i in range(len(refractive_indices)):
    #     idx = samples.Y == i
    #     plt.scatter(Y[idx, 0], Y[idx, 1], c=mpl_colors[i])
    # plt.show()
    #
    # Y = manifold.LocallyLinearEmbedding(n_components=2).fit_transform(samples.colors)
    # for i in range(len(refractive_indices)):
    #     idx = samples.Y == i
    #     plt.scatter(Y[idx, 0], Y[idx, 1], c=mpl_colors[i])
    # plt.show()
